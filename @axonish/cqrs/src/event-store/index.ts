...
const isSnap = (x: IEvent) => x.eventType === Snap.EVENT_TYPE;
const isNotSnap = (x: IEvent) => x.eventType !== Snap.EVENT_TYPE;
const toIEvent = (x: DomainEvent<unknown> | Snap) => x.toEventData();
...
const removeSnapField = (e: IEvent) => {
  if (e.eventType === Snap.EVENT_TYPE) {
    delete e.eventType;
  }
  return e;
};
...
export default class PgEventStore extends EventEmitter implements IEventStore {
  ...
  private snaps: PgTable<Snap> | null = null;
  ...
  async saveEvents(
    ...
    if (this.events !== null && this.snaps !== null) {
      for (const chunk of chunks) {
        ...
        const snapInsertResult = this.snaps.insert(snaps as Snap[]);
  ...
  async getLatestSnapshot(aggregateId: AggregateId): Promise<Snap | null> {
    if (this.snaps !== null) {
      const data: Snap = await this.snaps.findOne(
        { aggregateId },
        {
          limit: 1,
    ...
    if (this.events !== null && this.snaps !== null) {
      ...
      return events.map((event: IEvent) => {
        if (event.eventType && event.aggregateId) {
          if (event.eventType === Snap.EVENT_TYPE) {
